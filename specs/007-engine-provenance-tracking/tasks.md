...
- [ ] T001 Confirm on branch `007-engine-provenance-tracking`; run `pnpm typecheck && pnpm vitest run` and record the current pass/fail counts — expect 341 pass / 16 fail / 357 (see Phase 2 for the fix; do not treat this as a snapshot-gate baseline to route around)
...
- [ ] T002 [P] Update tests/unit/catalog/normalize.test.ts to match `normalizeBiasName`'s actual current contract (`{name: string, id?: string}`, no `confidence` field): remove all `.confidence` assertions; change `expect(result.id).toBeNull()` to `expect(result.id).toBeUndefined()`; for "should exact match by catalog id (kebab-case)," either change the input to a bias **name** (the function only matches by name, never by raw id — confirmed via src/catalog/normalize.ts) or remove the case and note id-exact-matching as a possible future enhancement, not a bug. Do not change src/catalog/normalize.ts itself — this is a test-only fix.
- [ ] T003 [P] In tests/unit/contracts/reflection.schemas.test.ts: remove or rewrite the two "should reject missing prompt_version" tests (`QuestionOutputSchema`, `AssessmentOutputSchema`) — `prompt_version` is intentionally `.optional()` per commit `4b27bb8` and ADR D003 (the orchestrator stamps it post-parse; the LLM never generates it). If coverage is still wanted, replace with an assertion that parsing succeeds when `prompt_version` is absent.
- [ ] T004 [P] In src/evaluation/compute-evaluation-metrics.ts (`isFalsePositive`, ~L110): change `(b.confidence ?? 0) > threshold` to `(b.confidence ?? 1) > threshold` — matches the test's documented intent ("defaults to 1.0"); an unscored bias in a no-bias story should count toward false positives, not be silently exempted.
- [ ] T005 In src/orchestrators/reflection/assessment.service.ts (T205 consistency block, ~L415–419): widen `parsed.noBiasDetected === undefined` to also catch `null` (e.g. `parsed.noBiasDetected == null`, or an explicit `=== undefined || === null` check). Root cause: src/parsers/repair.ts's `partialParseObject` (~L46) sets any field missing after a failed strict parse to `null`, not `undefined` — the existing check never fires on that path. Fixes both `tests/integration/assessment.test.ts` T505 and `tests/integration/two-phase-session.test.ts` Phase 1. **Different block than T017/T018/T019** (Phase 4) — confirm no line-range overlap before editing both in the same session.
- [ ] T006 [P] In tests/integration/two-phase-session.test.ts (Phase 2 test, ~L307): replace the hardcoded `expect(body.reasoningTrace.prompt_version).toBe("1.0.0")` with a comparison against the test's own `PromptRegistry` instance's `getVersion()` (or update the literal to the current value, `"1.3.0"`, if a dynamic reference isn't practical) — prevents this from going stale again on the next prompt version bump.
- [ ] T007 [P] In tests/integration/inngest-eval.test.ts (~L149): update `mockProvider.setResponse("asking 2-5", QUESTION_MOCK)`'s trigger substring — the current baked `src/prompts/reflection/question-batch/system.json` content reads "Generate 2–5 contextual follow-up questions" (different wording/dash), so the substring never matches and the mock falls through to "no response configured." Use a substring guaranteed to appear in the current content (e.g. a stable phrase from the GUIDELINES section) rather than re-hardcoding another fragile literal.
- [ ] T008 Run `pnpm vitest run` — confirm all 16 previously-failing tests now pass and zero new failures were introduced. This is the true green baseline; every "no regression" check in the rest of this plan means zero failures from here on, not "same known-red set."
