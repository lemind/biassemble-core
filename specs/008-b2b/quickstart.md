...
pnpm vitest run tests/integration/audit-verify.test.ts     # verify-golden-set.json, 19 cases
```
...
- VERIFY: ≥ 18/19 expected verdicts matched, zero false "contradicted" on period/scale/scope-only mismatches, zero false "supported" on a derived claim whose own stated magnitude is wrong (verify-016), correct partial credit at the magnitude boundary (verify-017), correct entailment on a necessarily-precluding negative claim (verify-018), correct partial credit on an adjacent-but-unstated inference (verify-019).
