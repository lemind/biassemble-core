...
pnpm vitest run tests/integration/audit-verify.test.ts     # verify-golden-set.json, 35 cases
```
...
- VERIFY: ≥ 39/40 expected verdicts matched, zero false "contradicted" on period/scale/scope-only mismatches, zero false "supported" on a derived claim whose own stated magnitude is wrong (verify-016), correct partial credit at the magnitude boundary (verify-017), correct entailment on a necessarily-precluding negative claim (verify-018), correct partial credit on an adjacent-but-unstated inference (verify-019), correct entailment on a compound negative claim without over-reaching to "contradicted" (verify-020), verbatim repros of both real arbitration incidents (verify-021/022), consistent verdicts across four synonym-paraphrased claims sharing one clause (verify-023–026), consistent "supported" across three redacted-placeholder claims (verify-027–029), correct per-item split on four structurally-parallel enumerated claims (verify-030–033), correct partially_supported on an on-topic-but-not-explicit going-concern claim (verify-034), correct contradicted when the note itself identifies the conflicting figure (verify-035), correct contradicted when the comparable evidence figure must be disambiguated from a same-sentence, different-measure figure (verify-036), correct contradicted on an entity claim whose own note asserts a contradiction (verify-037), correct contradicted regardless of the model's raw verdict when a real numeric disagreement is quoted as evidence (verify-038), correct contradicted on a quarter/date mismatch via the code-side temporal comparator (verify-039), correct partially_supported on a defined-term claim via the code-side policy, regardless of which direction the model guessed (verify-040).
