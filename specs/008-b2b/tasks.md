...
- [x] T039 Run `quickstart.md` end to end against a live (non-test) instance. **Fully completed against real production, happy path proven.** In order: (1) discovered this sandbox's network is geo-blocked from the Gemini API (`400 User location is not supported`) — not a code defect, confirmed by the pipeline's own error handling correctly surfacing it via `markFailed`/`GET /audit/:id`'s documented failure shape; (2) deployed this branch to production Vercel (`pnpm deploy`, explicit user approval — this branch was never pushed to `origin`, so `main` on GitHub remains pre-B2B; deployed directly from the local checkout via Vercel CLI) to get off the geo-blocked network; (3) found and fixed a stale `AI_CORE_API_KEY` in production (didn't match `.env` — rotated to the `.env` value with explicit user approval, redeployed); (4) confirmed Gemini itself is reachable from Vercel's network via the pre-existing, synchronous `/v1/reflection/question` endpoint (real generated questions, `modelName: "gemini-2.5-flash-lite"`, HTTP 200); (5) Inngest dispatch initially didn't fire for the new `auditRunJob` (Inngest's function registry needed a resync after deploying new functions — done manually by the user via the Inngest dashboard, not resolved via CLI/curl). With all of that cleared: submitted a real `POST /audit` against `https://biassemble-core.vercel.app` and polled `GET /audit/:id` to a genuine **`status: "complete"`** — 3 claims extracted (2 direct + 1 correctly-flagged `derived`/`synthesized` claim), all `verdict: "supported"`, `groundedness_score: 100`, `grounded_rate: 1`, real `model_revision`/`prompt_revision`/`pipeline_code_version` stamped in `meta`. This is the actual documented happy path, genuinely proven end to end — not a workaround. Two smoke-test audit rows created during the diagnostic process (before Inngest was resynced) were cleaned up by marking them `failed` with explanatory notes rather than left stuck at `"running"`.
