...
## Phase 4: Real-run verification ⚠️ Partially done

- [x] T008 (partial) — Ran the real (non-mocked) `GrounnelPipelineService` end-to-end via a one-off script wiring real `GeminiProvider`/`HybridSearchProvider`/`TavilySearchProvider` against the in-memory `FakeRedisHashClient` (bypassing only the Redis *infrastructure* requirement, not the pipeline logic itself — local `.env` has no Upstash Redis config, matching `server.ts`'s own documented conditional-boot behavior, so the HTTP route path was never an option locally). **Blocked short of the intended multi-source case**: local Gemini Search Grounding returned `400` and Tavily returned `403` — both external credential/config issues in this local dev environment, not caused by this change (consistent with `AGENTS.md`'s own "Current State" note, "Real eval runs: none yet, MockProvider only" — a pre-existing gap, not a regression). What DID get verified live: the real code path executed end-to-end (not mocked) and correctly degraded to `unsupported`/`evidence: null`/`citations: []` when search failed — a genuine, if minimal, confirmation that the new field behaves correctly under real (if unlucky) pipeline execution. Full verification against a real multi-source model response (SC-001) remains unverified beyond the unit/integration tests, which are themselves built from real, previously-documented live-eval citation shapes (D026's own incident history), not synthetic guesses.
