...
- [ ] **T037** VERIFY prompt v2.3.0 — retrieval-first STEP 1 reordering + a recency reminder, for recurring evidence-from-memory (D026, real live-eval finding: g05 recurrence, 2026-08-10)
  ...
  - **Verify:** prompt-only — no unit test for correctness possible, same "UNVALIDATED against a real model as of this commit" convention as every prior version bump. Confirm valid JSON + no hardcoded old-version references, then a live-eval re-run (watching g05) is the real verification.
