...
- **Never:** call Gemini's native `google_search` tool for retrieval (D019 §3 — structurally disqualified, not a quality question); read-modify-write a single Redis JSON blob for audit state (D019 §4 — use per-claim hash fields); net contradictions into the score (§13); let the passage-relevance filter's coreference gap get "fixed" with a hedged rule that doesn't actually resolve it (D019 §2 — leave it named, not papered over); add a *new* session/user mechanism, Inngest wiring, or state unrelated to D023's scope to `biassemble/backend` for Grounnel — it was a stateless pass-through only through P0 (D020 §5, explicit "Do not"). **Superseded, narrowly, by D023 §2**: `biassemble/backend` now reuses its *existing*, already-generic `sessions` table (tasks.md T028) — this is D020 §5 amended on its own terms (see spec.md's Post-P0 section), not the new/invented mechanism this line still correctly forbids.
...
- No Postgres dependency anywhere in this surface **through P0** (D019 §4); no session/user schema added to `biassemble/backend` for Grounnel **through P0** (D020 §5). **Both reopened, narrowly and on their own named terms, by D023** (tasks.md Phase 9, not yet built) — see Post-P0 below. This criterion is P0's bar, not a permanent one; it does not retroactively fail once D023 ships.
...
Real, non-mocked golden-set runs (D022) found and fixed three production bugs not caught by mocked unit/integration tests: a bare-array Gemini response silently nulling valid EXTRACT/VERIFY data (`repair.ts`, shared infra), a threshold-claim gap in gate #2 ("surpassed $X" only checked equality, not direction), and a verdict/reason binding mismatch (new `applyReasonConsistencyGate`, reusing audit's proven contradiction-language regex). A VERIFY prompt rewrite (v2.0.0) was also shipped and measured against the golden set — confirmed to fix the binding-mismatch class but **not** a bare "X, not Y" negation case (`g05`) even with a matching worked example in the prompt, an explicitly recorded negative result (D022 §3) so it isn't re-attempted blind. A code-side gate for that remaining gap shipped (D022 §4, tasks.md T021, `applyImplicitNegationGate`); a second known gap (multi-date role misclassification, `g04`) was investigated and closed as scoped-out — it's the only year-bearing claim in the golden set, no second case to justify a gate (tasks.md T022).

**Durable persistence (D023, tasks.md Phase 9, not yet built):** this surface currently persists nothing past Redis's 7-day TTL — no run history, no analytics, and this session found the search-fallback path (D021) had no durable tracking at all, only short-retention Vercel logs. D023 reopens D019 §4 on its own named triggers and adds a new `grounnel` pg schema (own tables, sibling to `core`/`audit`) plus reuses `biassemble/backend`'s existing anonymous-session mechanism — no login system added, matching how the reflection product already works. Redis stays the live-polling path unchanged; Postgres is additive.
