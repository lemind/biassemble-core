...
## §8. Addendum (2026-08-25) — the two rate-limiter gaps this ADR documented are now closed; a third, related one surfaced

§4 named IP rate limiting as defense-in-depth, not primary — true, but two implementation gaps meant it wasn't reliably enforcing even that: (1) `x-grounnel-client-ip` (added so real end-users behind `biassemble/backend`'s shared egress IP don't collapse into one bucket) was trusted from *any* caller holding `AI_CORE_API_KEY`, not just the proxy — live-confirmed exploitable (spoof the header, get a fresh bucket every request); (2) the limiter was an in-memory `Map`, which Vercel's multi-instance Lambda model doesn't share across invocations — also live-confirmed (a request landed on a fresh, empty bucket seconds after a different instance had already 429'd the same IP).

Both are fixed. `x-grounnel-client-ip` is now only honored alongside a second header carrying `GROUNNEL_INTERNAL_PROXY_SECRET`, a secret distinct from `AI_CORE_API_KEY` and known only to the trusted proxy — anything else falls back to `request.ip`, fail-closed. The limiter itself moved to the same Upstash Redis connection `GrounnelStore` already uses (D019 §4), via an atomic `INCR`-with-conditional-`EXPIRE` Lua script preserving the exact original fixed-window semantics (5/IP/hour) rather than switching to a sliding window.

**Found during that fix's own review, not yet acted on**: the `request.ip` fallback above is not actually a safe fallback. `buildApp()` (`server.ts`) sets `trustProxy: true`, which per Fastify's own implementation (`@fastify/proxy-addr`, confirmed by reading the installed source and reproducing locally) trusts every hop of `X-Forwarded-For`, including the client-supplied leftmost entry — so `request.ip` is itself spoofable by anyone who sends that standard header directly, no special knowledge required (more discoverable than `x-grounnel-client-ip` ever was). This affects the original code too, not just this fix; it was never exercised as a rate-limit bypass before because the in-memory limiter's cross-instance gap already made bypass trivial without bothering. Whether it's exploitable *in practice* through Vercel's actual edge (which may itself sanitize inbound `X-Forwarded-For` before the request reaches the Lambda) is unverified — that's a platform-behavior question, not a code-reading one. Left open deliberately rather than guessed at; tracked as a follow-up, not fixed as part of this addendum.
