...
1. `verdict === "unsupported"` — narrower than "not already contradicted": excludes `unverifiable` (a confidence downgrade this gate must not override) and `supported` (a `/code-review high` finding caught firing on a narrative "X, not Y" correction the model had already resolved correctly, e.g. "the mascot is Wildcat, not Tiger as it was previously known").
2. `reason` matches `,\s*not\s+<capitalized phrase>` → extract `Y`. Y's words must be capitalized (entity-shaped) — an earlier draft allowed any word and swallowed trailing lowercase words into the match ("not Canada to the United States" captured "Canada to the"), a real bug found in review and fixed by requiring capitalization.
3. `Y` word-boundary-matches inside `claimText`.
4. **Claim entity extractor**: at least one key term from `claimText`, other than any of `Y`'s own words, appears in `passageText`.

Condition 4 is named "claim entity extractor," not "proper noun detector" — it extracts candidate entities specifically from the claim text to use as a second-party discriminator against the passage, which is a narrower and more accurate description of what it does than general NER framing would suggest. **As built**, it reuses gate #4's own `extractKeyTerms` (`passage-filter.ts`) rather than a second, hand-rolled capitalized-word regex — an early draft used a raw `[A-Z][a-zA-Z]*` span matcher and it false-fired on sentence-initial "The" matching across unrelated sentences (caught by the gate's own unit tests, not by design review); `extractKeyTerms` already excludes common sentence-initial words for exactly this reason, so the fix was to reuse it, not to patch the new regex. A second review pass (`/code-review high`) found the exclusion compared `Y` as one string against `extractKeyTerms`'s single-word terms, so a multi-word `Y` ("United Kingdom") let its own constituent words count as the "second" entity — fixed by excluding `Y`'s words individually (`y.split(/\s+/)`), not as one string.
...
**Investigation done (tasks.md T022)**: `g04` is the only case in the 11-case golden set with a year anywhere in its claim text — no second candidate exists to even compare against, so the count is 1. Real fetched-passage text isn't cached locally (Inngest-triggered runs don't write local fixtures), but the reason text has reproduced the same 2-year-same-sentence shape identically across every real run, including a real Inngest run on 2026-08-07 (`correctRate: 0`, `below_correct_rate`). **Decision stands: not building a gate for this** — logged as a known, scoped-out limitation, not fixed. Revisit only if a future golden-set addition or real production data shows a second case with this shape.
