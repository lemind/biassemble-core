...
describe("GrounnelPipelineService (T010)", () => {
  ...
  it("T034 (reviewed finding): a successful retry appends its gate events to the original pass's, instead of discarding the original trace", async () => {
    ...
    const events = gateEventStore.calls[0]!.events;
    // 12 gates x 2 passes, plus D025 §5's retry-contradiction check and D030 §3k's telemetry-only
    // retry_decision summary. 11 until spec 015 G2 added affirmation_evidence.
    ...
    // Index 18: each pass is 12 gates (offset 6 within a pass), so pass 2 starts at 12.
    expect(events[18]).toMatchObject({ gate: "contradiction_evidence", verdictAfter: "contradicted", reason: null });
    ...
    expect(events[24]).toMatchObject({ gate: "retry_reconciliation", verdictBefore: "contradicted", verdictAfter: "contradicted", overridden: false, reason: null });
    ...
    expect(events[25]).toMatchObject({ gate: "retry_decision", verdictBefore: "unsupported", verdictAfter: "contradicted", overridden: true, reason: "evidence_null" });
  });
  ...
  it("T027/D023 §5: records exactly one grounnel_gate_events batch per claim, one entry per gate, in chain order", async () => {
    ...
    // Real claim: verdict starts and ends "supported" — none of the twelve gates should fire.
