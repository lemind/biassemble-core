...
describe("AssessmentService — per-bias engineSources derivation (D017)", () => {
  it("engine=retrieved: copies engine sources; a bias the LLM named alone gets [] (distinguishable via ragCase)", async () => {
    ...
    // per-bias output must NOT carry ragCase (storage-only, per data-model.md §2)
    expect((byName["Sunk Cost Fallacy"] as Record<string, unknown>).ragCase).toBeUndefined();
  });

  it("engine=unavailable: [] means unknown, disambiguated by the returned ragCase (not assessment-alone)", async () => {
    const { service, provider } = makeService(null);
    provider.setDefault({
